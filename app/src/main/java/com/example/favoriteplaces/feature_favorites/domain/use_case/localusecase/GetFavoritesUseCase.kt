...
class GetFavoritesUseCase(private val repository: FavoriteRepository) {
    ...
    ): Flow<List<Favorite>> = repository.getFavorites().map { sort(it, favoriteOrder) }

    fun observe(): Flow<List<Favorite>> = repository.getFavorites()

    fun sort(favorites: List<Favorite>, favoriteOrder: FavoriteOrder): List<Favorite> =
        when (favoriteOrder.orderType) {
            is OrderType.Ascending -> when (favoriteOrder) {
            ...
            is OrderType.Descending -> when (favoriteOrder) {
