...
After physical-owner verification of the completed 3.x development line, the owner elected a verified export, uninstall, fresh production install, and import for both household devices. The public application therefore starts at version 1.0.0/code 1 and builds as `Places-1.0.0.apk`. Room keeps the complete current schema internally numbered 4, but schema 4 is now the sole production baseline: historical schemas 1–3 and their runtime migration implementations/tests were removed. Keeping the internal number at 4 avoids treating any current database restored by Android before first launch as a downgrade. Backup format v4 and backward readers remain so reviewed household exports can still be imported. All future database shape changes must advance from schema 4 with a tested forward-only migration; all future app releases must increase Android's version code from 1 while using semantic public version names.
...
Release 3.2.9 restores the city heading inside grouped Saved cards. The city appears once at the top of each city/color group card, above the place rows, rather than being repeated in every row; the normal ungrouped cards still omit it. The heading is bold, single-line, safely truncated, and uses **City not set** for a blank legacy city. Automated placement/recreation coverage and live API-37 inspection passed, and the signed 3.2.8→3.2.9 update retained the original install timestamp and all seven release-emulator records. Room and backup remain version 4, with no database, Google request, permission, or saved-data change.
