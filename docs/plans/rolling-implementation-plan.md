...
- Production uses application ID `com.personal.favoriteplaces`, version code 1/version name 1.0.0, launcher label **Places**, and the permanent household signer. The old `com.example.favoriteplaces` package remains separate during recovery.
- Production begins with the complete current Room schema, internally numbered 4, as its sole supported database baseline. Historical Room schemas 1–3 and runtime migrations were removed after the owner chose a verified export/uninstall/fresh-install/import rollout; no destructive fallback exists. JSON backups remain format version 4 and retain readers for formats 1–4.
...
**Phase 12 launcher-name cleanup — 2026-09-13:** Release 3.2.10/version-code 15 retires the temporary side-by-side migration label **Places New**. The replacement package and permanent signer remain unchanged, while the Android launcher, app drawer, and system-facing application label now read simply **Places**. Unit tests, zero-error debug lint, release vital lint, and permanent-signed release assembly pass. Direct APK inspection confirms package `com.personal.favoriteplaces`, version code 15/version 3.2.10, application label **Places**, APK Signature Scheme v2 verification, and unchanged signer SHA-256 `a7800d55e3c0c4ea56cd822ee3dbc60bbdc2bc01c679d56c34acd6d6942291f4`. The final automatically named `Places-3.2.10.apk` has SHA-256 `015edbd8e299962068e7b0e85decd79180e5670c80a25ef76af47d2ada2da86c`. This is a resource-only product-name correction: Room, migrations, backups, saved records, API requests, permissions, and navigation are unchanged.

**Phase 12 release-artifact naming — 2026-09-13:** Release builds now derive their output filename from the embedded version and produce `Places-<version>.apk` directly. The implementation uses AGP 9.3.1's public `androidComponents`/`VariantOutput.outputFileName` API only for release variants; it does not copy artifacts, depend on internal task names, use deprecated `applicationVariants`, or alter package contents, versioning, signing, or debug outputs.

**Production 1.0 baseline reset — 2026-09-13:** At the owner's direction, the completed modernization is now published as app version name `1.0.0` and Android version code 1. This artifact is a fresh-install release and intentionally cannot update the already-installed version-code-14 development build; the owner will verify the 34-record export, uninstall, install production 1.0, and import through the app. Room remains internally at schema 4 because it is the complete current data shape and Android may restore databases before first launch. Schema 4 is now the only source fixture and production baseline; schemas 1–3, the three runtime `ALTER TABLE` migrations, migration-only tests, and the unused Room testing dependency were removed. No destructive fallback was added. Backup codec v4 and its v1–v4 readers remain because the verified household export is v4 and backward-readable files cost no database migration at runtime. The replacement contract test verifies all 14 production columns and null/non-null metadata storage from a newly created database.

The clean gate passes all JVM tests, zero-error debug lint, lint-vital, signed debug/release/test assembly, **32/32** API-37 emulator tests, and **25/25** offline recovery-tool tests. Static inspection finds no runtime Room migration registration or destructive fallback. The final `Places-1.0.0.apk` reports package `com.personal.favoriteplaces`, version code 1/version 1.0.0, label **Places**, and signer SHA-256 `a7800d55e3c0c4ea56cd822ee3dbc60bbdc2bc01c679d56c34acd6d6942291f4`; APK SHA-256 is `dc4bdcd4d658f0c8cb2838085248138cd7b4418b41c6ce9d8e106f0540ab01f7`. The sole Room schema fixture SHA-256 remains `1914a968f42f58b969488db1cc4ff0fd490fcc297c5d15657ec8db8690bfdd00`.

The permanent-signed production artifact also passed a clean release installation on the API-37 emulator. The exact old synthetic release package at version code 14 was removed from the emulator only, deleting its seven test records; the physical phone and its 34 records were not touched. A non-downgrade fresh install of `Places-1.0.0.apk` then reported version code 1/version 1.0.0, created its private database, rendered the expected **0 places / No saved places yet** state, and logged no Room, SQLite, migration, or fatal error. The deleted emulator records remain reproducible test fixtures and prior preservation evidence remains documented.
...
| 2026-09-13 | Made release artifact names automatic through AGP 9.3.1's public Variant API: each release now builds directly as `Places-<version>.apk`, eliminating manual renaming without creating a second artifact or changing debug builds. |
| 2026-09-13 | Reset the completed product to fresh-install production 1.0.0/code 1. Kept complete Room schema 4 as the sole internal baseline, removed schemas 1–3 and all runtime Room migration code/tests/dependency, retained v1–v4 JSON import compatibility, and passed unit/lint/signed-build, 32 device, and 25 recovery-tool tests. |
